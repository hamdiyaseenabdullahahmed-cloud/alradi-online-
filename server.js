...
app.put('/api/products/:id', async (req, res) => {
    if (!req.session.userId || req.session.user.role !== 'admin') return res.status(403).json({ error: 'غير مصرح' });
    const { name, price, oldPrice, discount, stock, image } = req.body;
    try { await run(`UPDATE products SET name=?, price=?, oldPrice=?, discount=?, stock=?, image=? WHERE id=?`, [name, price, oldPrice, discount, stock, image, req.params.id]); res.json({ success: true }); } catch { res.status(500).json({ error: 'حدث خطأ' }); }
});
