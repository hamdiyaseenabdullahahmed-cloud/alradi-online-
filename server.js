...
db.serialize(() => {
    ...
    db.run(`CREATE TABLE IF NOT EXISTS users (id INTEGER PRIMARY KEY AUTOINCREMENT, name TEXT NOT NULL, email TEXT UNIQUE NOT NULL, password TEXT NOT NULL, role TEXT DEFAULT 'user', phone TEXT, address TEXT, createdAt TEXT)`);
    ...
    db.run(`CREATE TABLE IF NOT EXISTS messages (id INTEGER PRIMARY KEY AUTOINCREMENT, userId INTEGER, sender TEXT, text TEXT, isAdmin INTEGER DEFAULT 0, timestamp TEXT, date TEXT)`);
    ...
    db.get("SELECT COUNT(*) as count FROM products", (err, row) => {
        if (row && row.count === 0) {
            const sample = [
                ['هاتف الرعدي برو X', 'هواتف', 2999, 3499, 15, 'أسود تيتانيوم', 'كاميرا 200 ميجابكسل شاشة 6.8', 10, 'https://picsum.photos/id/0/300/300'],
                ['سامسونج S24 الترا', 'هواتف', 4940, 5200, 12, 'رمادي تيتانيوم', 'قلم S-Pen سعة 512 جيجا', 7, 'https://picsum.photos/id/1/300/300'],
                ['سماعة إيربودز برو', 'إكسسوارات', 899, 1099, 18, 'أبيض ناصع', 'عزل ضوضاء وصوت محيطي', 15, 'https://picsum.photos/id/3/300/300'],
                ['عطر بلو دي شانيل', 'عطور', 4140, 4600, 10, 'كحلي غامق', 'رائحة خشبية فاخرة تدوم طويلاً', 5, 'https://picsum.photos/id/2/300/300']
            ];
    ...
    db.get("SELECT COUNT(*) as count FROM users", (err, row) => {
        if (row && row.count === 0) {
            db.run("INSERT INTO users (name, email, password, role, createdAt) VALUES (?, ?, ?, ?, ?)", ['المدير العام', 'admin@raadi.com', bcrypt.hashSync('admin123', 10), 'admin', new Date().toISOString()]);
        }
    });
    ...
    db.get("SELECT COUNT(*) as count FROM settings", (err, row) => {
        if (row && row.count === 0) {
            ...
            db.run("INSERT INTO settings (key, value) VALUES ('returnPolicy', 'استرجاع خلال 14 يوماً لعيب مصنعي')");
        }
    });
});

// دوال لتسهيل التعامل مع قاعدة البيانات
...
app.post('/api/categories', async (req, res) => {
    ...
    try { await run("INSERT INTO categories (name) VALUES (?)", [req.body.name.trim()]); res.json({ success: true }); } catch { res.status(500).json({ error: 'حدث خطأ' }); }
});
...
app.post('/api/register', async (req, res) => {
    ...
    try {
        ...
        await run("INSERT INTO users (name, email, password, phone, address, role, createdAt) VALUES (?,?,?,?,?,?,?)", [name, email, bcrypt.hashSync(password, 10), phone, address, 'user', new Date().toISOString()]);
...
app.post('/api/login', async (req, res) => {
    ...
    try {
        ...
        req.session.user = { id: u.id, name: u.name, email: u.email, role: u.role, phone: u.phone, address: u.address };
...
app.get('/api/users', async (req, res) => {
    if (!req.session.userId || req.session.user.role !== 'admin') return res.status(403).json([]);
    try { res.json(await query("SELECT id, name, email, role, phone, address, createdAt FROM users WHERE role != 'admin'")); } catch { res.status(500).json([]); }
});
...
app.get('/api/messages', async (req, res) => {
    if (!req.session.userId) return res.status(401).json([]);
    try {
        let list = req.session.user.role === 'admin' ? await query("SELECT * FROM messages ORDER BY id ASC LIMIT 150") : await query("SELECT * FROM messages WHERE userId = ? OR isAdmin = 1 ORDER BY id ASC", [req.session.userId]);
        res.json(list);
    } catch { res.status(500).json([]); }
});
app.post('/api/messages', async (req, res) => {
    if (!req.session.userId) return res.status(401).json({ error: 'سجل أولاً' });
    try { await run(`INSERT INTO messages (userId, sender, text, isAdmin, timestamp, date) VALUES (?,?,?,?,?,?)`, [req.session.userId, req.session.user.name, req.body.text, req.body.isAdmin ? 1 : 0, new Date().toLocaleTimeString('ar-EG'), new Date().toISOString()]); res.json({ success: true }); } catch { res.status(500).json({ error: 'خطأ' }); }
});
...
// صفحات التحكم والموقع
app.get('/admin', (req, res) => res.sendFile(path.join(__dirname, 'public', 'admin.html')));
app.get('/', (req, res) => res.sendFile(path.join(__dirname, 'public', 'index.html')));

app.listen(PORT, () => {
    console.log(`========================================`);
    console.log(`🚀 متجر الرعدي يعمل بنجاح على الرابط: http://localhost:${PORT}`);
    console.log(`👑 المسؤول: admin@raadi.com / admin123`);
    console.log(`========================================`);
});
